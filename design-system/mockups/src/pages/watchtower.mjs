...
const body = h(LayoutContent, {padding: 4},
  ...
  grid(3, 16, ...STAGES.map(([sid, name, rate, backlog, fail]) =>
    h('div', {key: sid}, stageCard({sid, name, rate, backlog, fail})))),
  ...
  grafanaCard({url: '#',
    note: '런 단위 drill-down(정확도·지연·correlation 분해)은 Grafana 대시보드가 담당 — '
  ...
  h('div', {style: {display: 'grid', gridTemplateColumns: '1.9fr 1fr', gap: 24,
    ...
    h('div', {},
      ...
      FAILURES.map(([reason, state, tone, doc, note]) =>
        h('div', {key: doc}, failureCard({reason, state, tone, ids: doc, note}))),
    ...
    h('div', {},
      ...
      SLOS.map(([name, sid, cur, target, window, tone]) =>
        h('div', {key: sid}, sloCard({name, sid, current: cur, target,
          window, verdict: 'TBD'}))))));
