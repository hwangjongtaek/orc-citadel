...
히어로 8종은 **전부 1920×720 (8:3)** 이다. 원본처럼 132px 고정 밴드에 `cover` 로 넣으면 **세로 23% 만** 보여 장면이 안 읽힌다. 그래서 히어로가 있는 페이지는 전부 8:3 을 지키고, **상한으로 본문 공간을 지킨다.**

| 유형 | 해당 | 히어로 | Layout height |
| --- | --- | --- | --- |
| 문서형 | Citadel Gate · Watchtower | `aspect-ratio: 8/3` · 상한 **360px** | `auto` (자연 스크롤) |
| 앱셸 | 나머지 6공간 | `aspect-ratio: 8/3` · 상한 **240px** | `fill` (패널 독립 스크롤) |
| 히어로 없음 | index · empty-states | 132px 평평한 그라데이션 밴드 | `fill` |

**상한은 필수다.** 8:3 을 상한 없이 두면 1519px 뷰포트에서 570px, 2560px 뷰포트에서는 **960px** 을 먹어 본문이 통째로 밀린다. 앱셸이 240px 로 더 낮은 것은 3열 본문에 세로를 남기기 위해서다 — 그래도 원본 132px 보다 장면이 훨씬 잘 읽힌다. 잘리는 곳은 천장 창살·바닥 카펫으로 장면에서 정보량이 가장 적은 부분이다.

`flexShrink: 0` 이 없으면 `height:'fill'` 인 앱셸에서 헤더가 눌려 히어로가 상한보다 더 잘린다.

**히어로 이미지가 없으면 평평한 밴드로 내린다.** 8:3 이 빈 그라데이션 덩어리가 되기 때문이다. `index` 의 `crest-hero.png` 는 **1024×1024 (1:1)** 이라 8:3 에 넣으면 2.7배로 확대돼 화면을 뒤덮었다 — 히어로에서 빼고 본문에 56px 마크로 두었다.
