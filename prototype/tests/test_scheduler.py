...
# --- nightly collection is dispatch-only ------------------------------------


def test_collect_task_dispatches_collection_without_promotion_or_snapshot(monkeypatch):
    """Async S2 consumer owns promotion; nightly must not race its partial corpus."""
    ...
    assert calls == ["collect", "metrics"]
    assert not hasattr(sr, "_promote_new_docs")
    assert not hasattr(sr, "_promote_zones")


def test_rebuild_zones_exposes_callable_api():
    """Full rebuild remains an explicit manual function outside nightly collection."""
