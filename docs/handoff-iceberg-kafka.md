...
1. **Small files / compaction — 쓰기 경로는 고쳤고, prod 재측정 대기 (2026-09-23).**
   아래 실측은 그대로 유효한 사고 기록이고, 그 원인인 **행 단위 커밋은 제거했다**
   — `CuratedZone.batched_writes()` 가 승격 배치를 테이블당 최대 2커밋으로 내린다
   (→ [03](./design/03-storage-and-data-model.md) §4 구현 메모). 동일 입력
   로컬 실측(25문서): curated 스냅샷 **702 → 35**, `mentions` **250 → 1**, 벽시계
   **76.4s → 1.9s**. `dup_signatures`(문서마다 밴드 후보를 되읽어 배치가 끊기던
   유일한 잔재)도 뒤이어 해소했다 — 배치 안 가시성을 메모리 밴드 인덱스로 옮겨
   **배치당 curated 스냅샷 58 → 10**(코퍼스 무관 상수)이다. 남은 것은 **전량
   재빌드 경로 미배치화**(코퍼스 전량을 한 버퍼에 담는 거래라 손대지 않았다). **prod 값은 배포 후 재측정 전까지
   미측정**이다 — 로컬 수치를 prod 수치로 옮겨 적지 않는다. compaction·snapshot
   expiry 는 이 재측정 뒤에 판단한다. 아래가 고치기 전 prod 실측이다.
...
6. **문서당 승격 비용이 폴 경계와 충돌한다 (2026-09-23 prod 실측).** 미승격 25건
   ...
   **후속 (같은 날, 쓰기 배치화 이후):** §8-1 의 커밋 배치화가 이 비용의 일부를
   같이 가져갔다 — 동일 입력 로컬 실측에서 25문서 승격 벽시계가 **76.4s →
   1.9s** 다. 다만 이것은 **로컬 SQLite catalog·file warehouse·25문서 코퍼스**
   수치이고, prod 의 6.01s/doc 은 REST catalog 왕복과 MinIO 쓰기, 그리고 183문서
   코퍼스를 되읽는 dedup·블록 재조정을 포함한다. 커밋 수가 줄면 REST catalog
   왕복도 함께 줄므로 prod 개선폭이 로컬보다 **클 수도 작을 수도** 있다 —
   **배포 후 prod 재측정 전까지 이 벽은 해소된 것으로 취급하지 않는다.**

   **추가 (dedup 쓰기 되올리기 이후, 로컬 스케일 측정):** 배치 크기를 25로 고정한
   채 코퍼스만 키우며 쟀다 — 문서당 비용 **코퍼스 0에서 0.036s, 225에서 0.112s**
   (고치기 전 같은 조건: 0.120s → 1.068s). 상수는 약 12배 줄었지만 **형태는 그대로
   선형**이고, 따라서 총비용은 여전히 O(N²)다. 기울기만 약 18배 완만해졌다 —
   1,000만 스케일을 단순 외삽하면 여전히 성립하지 않는다. 재프로파일에서 남은
   시간은 승격 로직이 아니라 **Iceberg 커밋(upsert)과 manifest planning** 이다
   (코퍼스 175 기준 배치 4.0s 중 upsert 2.0s·`plan_files` 1.5s) — 누적 파일·스냅샷
   수에 비례하는 비용이므로 **다음 수단은 compaction·snapshot expiry** 다(§8-1 이
   말한 "쓰기 경로 배치화가 선행" 순서를 이제 통과했다). 로컬 수치이며 prod 값은
   배포 후 재측정 전까지 미측정이다.

   **prod 1차 재측정 (배포 직후, 2026-09-23).** 문서 10건 **재승격** 22.2s =
   **2.217 s/doc**, curated 스냅샷 델타 **+4**(전부 `claim_candidates`). 정직
   경계: 이건 **재승격**이라 insert-if-absent 쓰기가 대부분 no-op 이 된다 —
   신규 승격의 **하한**이고 6.01s/doc(신규 25건)과 **직접 비교할 수 없다**.
   깨끗한 비교는 다음 nightly 의 신규 문서분이며, 배치 로그에 `elapsed=`·
   `s/doc=` 를 남겨 그 측정이 사람 손을 타지 않게 했다. 승격 공백 관측도 prod
   에서 동작 확인했다 — **gap 0, 0.6s**(238문서 코퍼스, REST catalog).

   **prod 2차 재측정 — 신규 문서 기준 (2026-09-25).** 09-24·09-25 nightly 가
   신규 **49문서**(21+28)를 승격했고 두 런 모두 `promotion_gap 0` 을 기록했다
   (공백 지표 첫 실운영 관측). 스냅샷은 **1,254 → 1,637(+383)**. 테이블별로는
   배치화가 확실히 먹었다 — `mentions` **+64행 / +4스냅샷**(사고 당시 589행에
   476스냅샷이던 최악 테이블), `dup_bands` +216행 / +6스냅샷. **그런데
   `claim_candidates` 가 +7행 / +344스냅샷**으로 새 최악이 됐다.
   원인은 배치화의 실패가 아니라 **배치가 만들어지지 않는다**는 것이다: 같은
   25문서를 로컬에서 1배치로 승격하면 **11스냅샷·1.2s**, 5배치로 쪼개면
   51·2.2s, 25배치로 쪼개면 **251스냅샷·13.8s** — 커밋 상각의 단위는 배치이고
   이득은 전적으로 배치 크기에 비례한다. prod 의 문서당 7.8스냅샷은 **유효 배치
   크기가 1**이라는 뜻이며, 수집이 문서를 하나씩(HTTP 레이트리밋으로 분 단위에
   흩뿌려) 발행하고 consumer 가 1초 폴에 잡히는 것만 묶었기 때문이다.
   `POLL_BATCH_SIZE=25` 는 상한일 뿐 하한이 아니다. 대응으로 consumer 에
   `LINGER_SECONDS=60`(+ 연속 빈 폴 `QUIET_POLLS=30` 조기 종료)을 넣어 폴 여러
   번에 걸쳐 배치를 적재한다 — `consume` 자체가 poll 이라 적재 중에도 그룹
   하트비트는 살아 있고, linger+처리는 폴 간격 예산의 절반 이하다(테스트 고정).
   **linger 효과 측정 (2026-09-26 nightly, 신규 16문서).** 두 배치로 처리됐다 —
   `batch=1 promoted=1 elapsed=12.9s s/doc=12.94` 와 `batch=15 promoted=15
   elapsed=17.1s s/doc=1.14`. **배치가 실제로 만들어졌다**(직전에는 문서당 1배치).
   스냅샷은 **1,637 → 1,669 = +32, 문서당 2.0** — linger 이전 같은 조건(신규
   문서 nightly)의 **7.8/문서** 대비 3.9배 개선이고, 16문서가 16배치로 쪼개졌다면
   약 +256 이었을 자리다. 최악이던 `claim_candidates` 는 문서당 **7.0 → 0.69**.
   `promotion_gap` 은 3일 연속 0.

   **정직 경계:** `s/doc=1.14` 를 6.01s/doc 과 1:1로 비교하면 안 된다 — 이 15문서
   배치는 산출이 적었다(mentions 2·claims 0). 문서 내용에 따라 승격 비용이 크게
   흔들린다는 §8-6 의 관측은 그대로다. 반면 **문서당 스냅샷 7.8 → 2.0 은 같은
   조건(신규 문서 nightly) 끼리의 비교라 유효**하다. 또 `batch=1` 이 하나 남은
