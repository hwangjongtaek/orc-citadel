...
> 옮겨 적지 않는다.
>
> **후속 (dedup 쓰기 되올리기, 같은 날):** 위 실측에서 `dup_signatures` 만 문서당
> 1커밋으로 남았던 원인은 `_dedup_against_corpus` 가 문서마다 밴드 후보를 존에서
> 되읽어 미커밋 서명을 강제로 내렸기 때문이다. 프로파일 결과 그 루프가 **배치
> 43.6s 중 40.5s** 를 쓰고 있었고, 그중 약 25s 가 되읽기가 유발한 flush 였다.
> 배치 안에서 이미 처리한 문서는 존이 아니라 **메모리 밴드 인덱스**로 보게 하고
> 서명 영속을 루프 밖으로 미뤘다 — 가시성 규칙은 그대로다(문서 i 는 앞선 0..i-1
> 을 본다). 실측: 코퍼스 175건 기준 배치 **26.7s → 2.3s**, 문서당 **1.068s →
> 0.091s**, 배치당 curated 스냅샷 **58 → 10**(코퍼스와 무관한 상수).
>
> 남은 비용의 형태는 바뀌지 않았다 — 문서당 비용은 여전히 코퍼스에 **선형**이고
> (코퍼스 0→225 에서 0.036 → 0.112 s/doc) 따라서 총비용은 O(N²)다. 기울기가
> 약 18배 완만해졌을 뿐이다. 재프로파일에서 남은 시간은 **Iceberg 커밋(upsert)과
> manifest planning** 으로 옮겨갔다(배치 4.0s 중 upsert 2.0s·plan_files 1.5s) —
> 즉 누적 파일·스냅샷 수에 비례하는 비용이며, 이제서야 compaction·snapshot expiry
> 가 다음 수단으로 유효해진다(쓰기 경로 배치화가 선행이라던 순서 그대로).
>
