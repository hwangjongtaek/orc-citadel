...
> **구현 메모 (Phase 5 — 다국어 ER, 2026-08-12):** `multilingual_er.py` — 같은 개체(기관)가 여러 언어·스크립트로 등장할 때(TSMC ∽ 台積電, §1.2 `aliases`)의 **교차-스크립트 해소 전단부**를 봉인 (스크립트 탐지·다국어 정규화·교차-스크립트 후보, ADR-507 precision-first). **스크립트 탐지:** `detect_script` — codepoint 다수결로 표면형 지배 스크립트(latn/cjk/other, 결정적). **다국어 정규화:** `norm_name` — **NFKC + 조합 발음구분부호 제거 + (로마자) 소문자·공백·구두점 축약 / (CJK) 공백 제거** → blocking key `norm_name`(§2.1) 의 교차-스크립트 변형 기반. **교차-스크립트 bridge:** `multilingual_alias_index` — caller-curated alias(TSMC↔台積電) 를 norm→entity 로 인덱싱(결정적). `multilingual_block_keys` — bridge 를 타고 **교차-스크립트로 같은 개체**를 가리키는 후보를 같은 bucket 으로 유도(§2.1 blocking 확장). **후보 제안:** `propose_multilingual_candidates` — 스크립트 다른 개체 후보를 **`POSSIBLY_SAME_AS`(kind=multilingual-script)** 로 제안, `multilingual_merge_status` — **교차-스크립트는 항상 POSSIBLY**(ADR-507 — 결정적 외부식별자 exact match 만 자동 병합, transliteration 추론으로 병합 금지). **honest-gap(§6.2):** 교차-스크립트 동치는 caller alias bridge 신뢰만큼만 — **bridge 없는 표면형은 교차 후보를 추론하지 않음** (transliteration 은 모호·비결정적). read-only(불변식 §3-3)·결정적·mock/실측 격리 — 병합·SAME_AS 이벤트 발행은 ER 캐스케이드(호출자) 몫. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_multilingual_er` 신규 30개(스크립트 탐지·정규화·bridge 인덱스·blocking 유도·후보 제안·항상 POSSIBLY·honest-gap·read-only·결정성) — 스위트 842→**872개 통과**(회귀 0). 다음: Phase 5 는 ontology migration 자동화(02).
