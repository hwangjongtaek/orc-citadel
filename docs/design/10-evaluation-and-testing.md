...
> **구현 메모 (Phase 5 — impact graph 부분 재계산, 2026-08-12):** `impact_graph.py` — §4.5 **partial recomputation 대비 절감** 의 **범위 도출층**을 봉인 (full rebuild 와 대비되는 *무엇을* 재계산할지 도출 — 기존 `recompute_bench`(벽시계 계량)·`graph_incremental`(실재적용) 과 결합). `impact_scope` — 전파 엣지 BFS **영향 하류 집합**, `partial_recompute_events` — full 중 영향 scope 만 추림, `partial_vs_full` — **이벤트 수 기반** partial 절감 ratio(partial=0 → `empty_scope` honest-gap), `evaluate_partial_recompute` — ratio > `PARTIAL_RATIO_GATE=5.0` → `saving_meaningful` + **slo-gate**(§1.4 CI 비차단 nightly). read-only·결정적. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_impact_graph` 신규 29개 — 스위트 813→**842개 통과**(회귀 0). 다음: 다국어 ER(05).
...
> **구현 (Phase 4 — 증분 graph update·재구축/증분 비율 재측정, 2026-08-12):** `graph_incremental.py` — 06 §7.2 incremental rebuild 실경로 봉인 + #14 recompute benchmark 이어받아 **재구축/증분 비율 재측정**. `extract_events(graph)` 그래프→mutation 이벤트 역직렬화(결정·read-only) · `apply_incremental(base, delta)` base 위 delta subgraph 만 재적용 (그래프 쓰기는 `GraphService.apply` idempotent 경로만) · `incremental_rebuild_bench(eval executor mock)` full(incremental ratio(10 §4.5, 06 §9 3항) · `evaluate_incremental_bench` 완료조건 **ratio > INCREMENTAL_RATIO_GATE(10×)** → `saving_meaningful` + slo-gate(§1.4 nightly 비차단) · `compute_graph_slo` **DoD ② "신규 문서가 SLO 내 그래프 반영"** 게이트(`GRAPH_SLO_MS=60s` p95, 미측정 honest-gap §6.2). **정합성 계약:** incremental 재적용 그래프 == full replay 그래프 (불변식 §3-1 결정적). read-only·결정적·mock/실측 격리. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_graph_incremental` 신규 16개(extract·정합성 동치·base 보존·idempotency·ratio 게이트·SLO·read-only·결정성) — 스위트 625→**644개 통과**(회귀 0). 다음: 대량 embedding·LLM batch inference(07·08).

> **구현 (Phase 4 — 대량 embedding·LLM batch inference, DoD ①, 2026-08-12):** `batch_inference.py` — 08(검색)·07(LLM) inference 단계 확장 지점 봉인 (mock/실측 격리, DoD ① 처리 시간·비용). **embedding (08 §2.3, ADR-809 — 08 소유):** 핀 `BAAI/bge-m3`(1024-dim cosine), `embeddable_kind` — segment/claim 만(ADR-802), `embedding_index_version` model:dim 축약(§2.1 전량 reindex 트리거), `embed_texts` 일괄 임베딩(executor 주입, `embedding_model`·`index_version` 부착). **LLM batch (07 §2.2):** `BATCH_TIER=claude-sonnet-5`, `batch_infer` Message Batches 일괄(미주입 mock 은 항목 `pending` — honest-gap §6.2 진공 통과 금지), `batch_inference_cost` = 순차 × `MESSAGE_BATCHES_DISCOUNT(0.5)` — **50% 절감**(비-latency-민감). **DoD ① 측정:** `inference_throughput`(10 §1.4 재사용) + `compute_inference_slo`(`INFERENCE_SLO_MS=60s` **slo-gate**·CI 비차단, 미측정 honest-gap). read-only(불변식 §3-3)·결정적·mock/실측 격리. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_batch_inference` 신규 26개(핀·embeddable·index_version·embed 결정성/executor·batch pending·비용 절감·SLO 게이트·read-only·결정성) — 스위트 644→**670개 통과**(회귀 0). 다음: ClickHouse 분석 승격(11, 확장 게이트).

> **구현 (Phase 4 — 100만 처리 시간·비용 공개 + SLO graph 반영, DoD ①②, 2026-08-12):** `phase4_report.py` — #14–#17 이 봉인한 계약을 **DoD ①② 측에서 통합 공개** (중복 구현 없음, 10 §4.5·§1.4 → 01·06). **DoD ① — 100만 처리 시간·비용 공개:** `project_time_to_scale(per_doc_ms, docs, target=1M, speedup)` — 선형 × 병렬(batch #14) 투영 · `project_cost_to_scale(cost_per_doc, target=1M, use_batch)` — 순차 vs `batch_inference_cost`(Message Batches **50% 절감**, #17 재사용). 이 값은 **결정적 모델 투영(추정치)** 이지 실측이 아니므로 `measured=False` 로 명시 (honest-gap §6.2 — 실측 부재가 공개 값의 근거). **DoD ② — 신규 문서 SLO 내 graph 반영:** `graph_reflect_slo` 가 `compute_graph_slo`(#15, p95 ≤ GRAPH_SLO_MS)·`compute_slo_gate`(#14, 노드당)·증분 완료조건(ratio > 10×) 을 통합 판정(각 분류 재노출). `phase4_dod_report` — **`all_measured`** 로 전 구성요소 실측 여부를 정직히 노출(부분 미측정 시 False — §6.2). read-only(불변식 §3-3)·결정적. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_phase4_report` 신규 18개(시간/비용 투영·batch 할인·honest-gap·SLO 통합·all_measured·read-only·결정성) — 스위트 689→**707개 통과**(회귀 0). 다음: Phase 4 DoD ①② 통합 검증 + 완결 블록업.

> **구현 (Phase 4 — DoD ①② 통합 검증, 2026-08-12):** `test_phase4_dod.py` — #14–#18 이 봉인한 엔진을 실제로 구동해 **DoD ①② 종합 검증** (read-only·결정적·mock/실측 격리). **DoD ① — 100만 처리 시간·비용 공개:** `project_time_to_scale`·`project_cost_to_scale`(1M 투영, 병렬·**Message Batches 50% 절감**) 조합해 `phase4_dod_report` 로 공개 — 결정적 모델 투영이 실측이 아니므로 정직히 `measured=False`(honest-gap §6.2), SLO 위반은 `slo-gate` 비차단(§1.4·§6.3). **DoD ② — 신규 문서 SLO 내 graph 반영:** 실제 `GraphService` 로 `_g(n,e)` 생성 → `extract_events`·`apply_incremental` 구동해 **incremental 재적용 그래프 == full replay 그래프** 확인(불변식 §3-1 정합성)·base 원본 불변(read-only §3-3)·증분 **ratio > 10×** 완료조건·`compute_graph_slo`(p95 ≤ GRAPH_SLO_MS) `ok` 판정 — DoD ② 종합. 결정성(동일 입력·동일 벤치). TDD — 신규 12개 — 스위트 707→**719개 통과**(회귀 0). [§3](./ROADMAP.md) Phase 4 완결 마킹.
