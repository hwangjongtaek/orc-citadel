...
| SLO-05 | source 수집 성공률 | `≥ 99%` (deferred) | 7d rolling | **실측 (2026-08-12)** — SEC+RSS 50건 `{success_rate:1.0, n=50, measured:True, within_slo:True}` (목표 ≥99% 기계적 판정 기록) · **표본 n은 런별 독립**(`slo_log` in-memory — 런 간 누적 없음, n≈arXiv `total`+RSS'): A5(2026-08-18, total=400) n=440 · A6(windows=10, total=1500) n=1,540 · A7(windows=16, total=2000) n=2,040 · A8(windows=20, total=2000) n=2,040 — 전부 `{success_rate:1.0, within_slo:True}` (성공률 1.0 유지). **A19(2026-08-19, 후보 2):** RSS 3 source + BIS sitemap URL-skip 런 — **raw 104,905→104,906(+1, AMD 신규 1)** · SLO-05 `n=77`(RSS 40 시도+sitemap 37, in-memory 런별) `success_rate:1.0 within_slo:True` (skip 은 시도 아님). ~~런 간 누적/추가 축적~~(정정 — in-memory 로 런 간 축적 구조적 불가, §2.3 실측 메모) |
| SLO-06 | schema validation 통과율 | `≥ 95%` ✅ 확정(잠정) | 7d rolling | **실측 (2026-08-19, 무비용 LLM)** — `claude_judge` verdict 스키마 검증, supply chain pair 10건(contradiction 5·canonical 5) 무비용 `bunker-flash` LLM 실측 → **`pass_rate 1.0`(10/10) `within_slo:True`**(≥95%) · measured=True n=10. 실측 과정에서 **설계-프롬프트 불일치 발견·해소**: contradiction 프롬프트가 `evidence_spans` 를 모호한 `<지지 구간>` 으로 정의(05 §5.2 는 dict) → LLM 이 str 리스트 반환 → 검증 크래시(수정) 및 전수 실패(0/5) — 프롬프트를 설계 05 §5.2 dict 구조(`doc_id`·`char_start`·`char_end`)로 정합 후 1.0. **표본 확대 (A24):** 동일 무비용 LLM, supply chain claim-id pair **30건(contradiction 15·canonical 15)** 실측 → **`pass_rate 1.0`(30/30) `within_slo:True` `measured:True` n=30 n_unknown=0** — A21 결함 수정 후 canonical·contradiction 양 경로 **둘 다 100% 통과 재확정**(None verdict 0). 판정은 **확정(잠정)** — 유한 샘플 실측, 런 축적(운영 7d rolling) 후 재확정. **7d 축적 실현 (A26):** `slo06_accum` — nightly 마다 `record_schema` 개별 결과를 `data/slo06_accum.json` 에 append, 최근 7일 누적으로 `accum_slo06` 이 `schema_pass_rate`→`evaluate_slo06` 재확정 판정. `scripts/nightly_slo06.py` 가 매일 소표본 실측·append — 7일치 누적 시 확정(잠정)→확정 재확정 근거 (실측 검증 cap=10 → pass_rate 1.0·within_slo True). **dispatch 계층 (A27):** `nightly_slo06`·`nightly_collect` 는 **cron 대신 APScheduler 상주 스케줄러**(`scripts/scheduler_runner.py`, BackgroundScheduler + SQLAlchemyJobStore, 단일 호스트·단일 활성 스케줄러) 가 07:37/07:07 로 dispatch — 셸 cron hang 우회, 영속 job store 로 재시작 후에도 스케줄 유지 (celery beat 대체, A26 `.cron` 전환). **실측 점검 (2026-09-02, Phase 6 deferred 점검):** 7d 누적 현황 점검 — `slo06_accum.json` 에 5일치(118건: 8/22 10, 8/26 30, 8/27 30, 8/28 30, 8/31 18) 누적으로 7일치 미달(결측일 발생). 8/27 이후 환경 LLM backend(`glm-4.7-flash`, reasoning)의 reasoning_tokens 소비로 인한 max_tokens=512 초과 및 markdown fence 출력으로 schema validation fail 발생 확인 → **확정(잠정) 유지** (honest-gap §6.2, 7일 누적 및 모델 파라미터 정비 후 재확정 대기). |
...
> **실측 메모 (Phase 6 — SLO-06 무비용 LLM 실측 + 설계-프롬프트 불일치 해소, 2026-08-19):** 사용자 결정 "현재 설정 LLM(무비용 bunker-flash)으로 대체 가능" — 비용 유보 때문에 유보하던 SLO-06(LLM output schema 통과율) 실측을 **무비용 LLM으로 수행**. `.env`(`LLM_PROVIDER=litellm`·`LLM_BASE_URL=https://llm-proxy.datamaker.io`·`LLM_MODEL=bunker-flash`) 를 로드해 `build_llm_client()` → `OpenAICompatibleClient`(bunker-flash, vllm 서빙) 실제 호출 확정·무비용. **실측 발견 (2건 결함):** (1) `validate_contradiction_verdict` 의 `evidence_spans` 파싱 `s.items()` 가 **dict 아닌 항목(str)에서 `AttributeError` 크래시** — ADR-704·07 §7("구조화 실패 → None 유도·후보 유지") 위반, 검증 실패가 예외로 전파 → **방어적 수정**(비-dict 항목 → None). (2) **설계-프롬프트 불일치**: `_CONTRADICTION_SCHEMA_INSTRUCTION` 이 `evidence_spans` 를 `<지지 구간>` 로 모호 정의 → LLM(bunker-flash) 이 **str 리스트로 체계적 반환** → contradiction verdict **전수(0/5) 스키마 실패** (canonical 은 evidence_spans 미요구라 5/5 통과 — 통과율 0.5·`slo-gate`). **수정:** 프롬프트를 **설계 05 §5.2 dict 구조**(`[{"doc_id","char_start","char_end"}]`)로 명시 + "dict 객체" 요구. **재실측 (동일 10건):** canonical 5/5 + contradiction 5/5 → **`pass_rate 1.0` ≥95% `within_slo:True`** · 무비용(usage 2190/987 tokens, calls 10). **판정: SLO-06 확정(잠정)** — 10건 샘플 실측 기준, 운영 7d rolling 축적 후 재확정 대상. **Spec 판정: 1.1.0 유지** — 검증 함수는 07 §7/ADR-704 기존 계약 이행(버그 수정)·프롬프트는 설계 05 §5.2 미이행 이행(verdict/conflict_type 라벨·구조 무변경, additive) — 계약/스키마 추가 아님. **TDD:** `test_contradiction_verdict_malformed_evidence_span_returns_none`(Red→Green)·`test_contradiction_prompt_specifies_dict_evidence_spans` 신규 — 스위트 **1038→1040 Green**(회귀 0). read-only(저장소 쓰기 없음)·무비용 LLM(비용 지출 0). 이 실측은 **비용 유보가 SLO-06 실측을 막던 것을 무비용 LLM으로 해소**한 사례 — SLO-06 이 유일 비용-유보 해제 대상이었으나 무비용 대체로 실측 완료. 다음: SLO-06 는 운영 런 축적(7d)으로 재확정, 나머지 deferred(#4 라벨링·SLO-07) 는 모집단/라벨링 종속으로 유지.
...
> **실측 메모 (Phase 6 — SLO-05 표본 n=440→1,540, A6 런, 2026-08-18):** A6 수집 확대 런(아래)의 `slo_log` 주입으로 sealed 하니스 `collect_success_rate` **`{success_rate:1.0, n_success:1,540, n_attempt:1,540, n_unknown:0, within_slo:True}`** measured=True — **표본 n=1,540**. ⚠️ **정정 (2026-08-18, A8 런 관측):** `slo_log`(`SloObservationLog`)는 **순수 in-memory** — 런 간 누적 없음. n=1,540 은 **이번 런 자체 시도 수** = arXiv `total=1500`+RSS 40 (부정확한 "A6 표본 n=440 → n=1,540 (3.5×)·추가 축적" 서술 대체). arXiv windows=10 의 실제 fetch 시도/성공 전부가 기록되어 1,540 시도·성공률 1.0. `n_unknown=0` — 부재 없는 진짜 시도/성공. 성공률 1.0 추정치는 여전히 표본 기반 — 일반화 한계 인지. n 을 독립적으로 키우려면 런 별 arXiv `total` 파라미터를 조절(런 간 축적과 무관) — 다음 런에서 보강 가능.

> **실측 메모 (Phase 6 — SLO-05 표본 n=1,540→2,040, A7 런, 2026-08-18):** A7 수집 확대 런(아래)의 `slo_log` 주입으로 sealed 하니스 `collect_success_rate` **`{success_rate:1.0, n_success:2,040, n_attempt:2,040, n_unknown:0, within_slo:True}`** measured=True — **표본 n=2,040**. ⚠️ **정정 (2026-08-18, A8 런 관측):** `slo_log` 는 **순수 in-memory 참조(불변식 §3-3)·런 간 누적 없음** — 설명에 "`SloObservationLog()` 는 파일·DB 영속이 없는 read-only 로그(코드 검증)" 를 기록. n=2,040 = **이번 런 자체 시도 수** = arXiv `total=2000`+RSS 40 (부정확한 "표본 n=1,540 → n=2,040 (1.3×)·추가 축적·다음 런 RSS refresh 로 계속 축적" 서술 대체 — in-memory 라 RSS refresh 로는 축적 불가). **동반 수집 포화 관측 (진실성 유지):** A7 arXiv skip 1,998/2,000 — 수집이 자연 포화에 접근, 실질 신규는 RSS 유한에 의존. **A8(2026-08-18, windows=20):** arXiv **+0 skip 2,000** — 2025-03 까지도 전 연대 소진 재확인, RSS 신규 +10 → raw 104,725→104,735. 성공률 1.0 추정치는 여전히 표본 기반 — n 을 독립적으로 키우려면 런 별 arXiv `total` 조절.
...
> **실측 메모 (Phase 6 deferred 항목 점검 및 실측 런, 2026-09-02):** `docs/handoff-phase6-deferred.md` 위임 실행.
> 1. **SLO-06 (7d rolling 재확정):** 착수 조건(7일치 누적) 미충족 확인 — `data/slo06_accum.json` 에 5일치(118건: 8/22 10건, 8/26 30건, 8/27 30건, 8/28 30건, 8/31 18건)만 축적됨. 8/27 이후 LLM 모델 변경(`glm-4.7-flash`, vllm)으로 1200+ reasoning tokens 소비 및 max_tokens=512 출력 잘림, markdown code fence(```json) 출력으로 스키마 검증 실패 확인. 정직하게 **확정(잠정) 유지** 및 7일 축적 대기 (honest-gap §6.2).
