...
- **실신호 본문 재판정 (2026-08-12, 가동 postgres→Neo4j)**: object 바인딩(`8838aeb`)으로 **실신호 본문 소스**(전용 반도체 언론 RSS — nvidianews 55·semiengineering 73, 총 128건)에서 공급망 엣지가 실존하는 그래프로 **Q4 재판정**. `signal_source_runner`(재현 러너)가 전용 신호 소스 선택→파이프라인→postgres SoT(log 243 mutations)→replay **194노드·49엣지**(announces 33·supplies 16, **quarantine 0**)→Neo4j 적재(420ms)·조회 **p95=0.66~1.16ms**(970회). **3개 게이트 전항 PASS, Memgraph 트리거 없음** — 이로써 이전 판정이 유보한 "1e6/p95-500ms 는 실신호 본문 소스 확보 후 재판정"의 실신호 재료가 확보되어, **실데이터 엣지 그래프에서 SLO 미달 없음**을 확인. 합성 10만 단위 조회 보증은 그대로 유효.
...
> **구현 메모 (Phase 5 — hot/cold graph 분리, 2026-08-12):** `graph_temperature.py` — 10M Challenge 그래프 확장의 **접근 온도 기반 계층 분리**를 봉인 (ADR-603 — 초기 논리 라벨 분리 → 확장 시 **물리 분리**, Q4 게이트 — 노드 ≥ `NODE_GATE=1e6` 시 분리/아카이브 트리거). **온도(§8 접근 패턴):** `temperature` — 최근성(`now - last_access ≤ hot_window_sec`) → hot/cold, **미접근(None) → None**(미측정, honest-gap §6.2). **분할:** `partition_tiers` — access_stats 를 hot/cold/unknown 으로(결정적), `tier_map` — unknown 을 **보수적 기본(cold)으로 할당**(측정과 구분, hot 예산 보수 유지). **hot 예산:** `hot_resident_count`. **Q4 물리 분리(§9 게이트):** `cold_archive_decision` — 노드 ≥ 1e6 이면 `archive_needed=True`, cold 아카이브 = `node_count - hot_resident`(hot 상주 보존)·`cold_ratio`. **조회 라우팅(§8.3):** `route_query` — hot → 상주(빠름), cold → 아카이브(느림 원장 노출), tier None → `not-measured`; 지연 SLO(`DEFAULT_SLO_MS=50ms`) 초과 → `slo-gate`(비차단, 10 §1.4). **승격:** `mark_accessed` — 새 stats dict 반환(입력 불변), 실제 접근 기록 갱신은 호출자 몫 (read-only §3-3). 접근 stats·지연은 호출자가 주입(mock/실측 격리). hot_window 는 placeholder(사용자 정의). **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_graph_temperature` 신규 28개(온도·분할·unknown 할당·hot 예산·아카이브 게이트·라우팅+SLO·승격 read-only·결정성) — 스위트 785→**813개 통과**(회귀 0). 다음: impact graph 부분 재계산(§7.2·§5.2).

> **구현 메모 (Phase 5 — impact graph 부분 재계산, 2026-08-12):** `impact_graph.py` — §7.2·§5.2 **영향 하류 subgraph 도출**을 봉인. **프로파게이션 모델:** 전파 엣지(`PROPAGATION_TYPES` = ABOUT/SUPPORTS/CONTRADICTS/SUPERSEDES/SAME_AS) 로 연결된 **연결 클러스터는 상호 영향**(양방향 — §5.2 재지정 파급), hops·include_types 로 범위 한정. **범위 도출:** `impact_scope` — target→전파 BFS **영향 노드 집합**(결정적·read-only). **범위 materialize:** `impact_subgraph` — scope 노드 + **양끝이 모두 scope 인 내부 엣지만**(부분 재계산 섬, 경계 제외). **부분 재계산 이벤트:** `partial_recompute_events` — full 이벤트 중 scope 내 노드·내부 엣지만 추림(원순서 보존 — replay 정합). **절감 계량(10 §4.5):** `partial_vs_full` — full/partial 이벤트 수·ratio, **partial=0 → ratio None + empty_scope**(honest-gap §6.2 — "아무것도 안 함"≠유의미 절감). **게이트:** `evaluate_partial_recompute` — ratio > `PARTIAL_RATIO_GATE=5.0` → `saving_meaningful` + `slo-gate`(10 §1.4 CI 비차단 nightly), empty-scope → 절감 미인정. 영향 범위·부분 이벤트는 산출물(read-only §3-3) — 실제 재적용은 `GraphService.apply` 경로만. 결정적·mock/실측 격리. **스키마·계약 변경 없음 → Spec 그대로(0.1.9).** TDD — `test_impact_graph` 신규 29개(범위 BFS·선택·hops·타입 필터·subgraph 섬·부분 이벤트 부분집합·비율·empty-scope honest·게이트·read-only·결정성) — 스위트 813→**842개 통과**(회귀 0). 다음: Phase 5 는 다국어 ER(05).
