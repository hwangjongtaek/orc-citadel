...
- **배선 상태 (2026-08-18, A9):** `collect_large.SOURCES` 에 구현된 초기 소스는 NVIDIA(#4)·AMD(#6)·SemiEngineering(#5)·arXiv(#2)·SEC(#1) 이었고 **#3 CHIPS/NIST 는 스펙 지정만 되어 미배선**이었다. **A9 런으로 #3 를 `SOURCES` 에 배선**(`gov-chips-nist`, `("rss", "https://www.nist.gov/news-events/electronics/rss.xml")`) — 기존 `collect_rss` 폴리트 러너·`_save_zone`(`base/gov-chips-nist/doc/…` 별도 zone) 그대로 재사용. **실측 (2026-08-18, A9):** `saved=40` 전부 신규 — raw 성장의 실질 주축(nvidia/amd/semiengineering 은 content-hash 중복 다수였음). `gov-public` 라이선스(store·재배포 무제약). 실제 정부 커버리지: CHIPS법 자금·수출통제 Letters of Intent 등 정책 문서. SLO-05 반영: 이 런 slo_log 로 성공률 1.0 확대 시도 수요에 기여(RSS`source_type=gov` 도 `record_collect` 성공 기록). **signal 스코프 판정 (2026-08-18, A10):** `signal_density` 실측 — gov 40건 **claims 1·density 0.025**(=비신호 arXiv 수준) vs signal 3종 0.63~5.9 → **`signal_source_runner.SIGNAL_SOURCE_IDS` 미포함 유지**(신호 수율 스케줄 §1.1·11 §1.4 "저신호 오판 금지" 정합). raw 수집(gov 유효)과 실신호 재현(부적격)은 **별개 스코프**로 확정. details: ROADMAP §5 A10.
