...
> **구현 (Campaign Ledger REPORT — 2026-09-21):** worker 성공 경로를 `PLAN → RUN → SYNTHESIZE → AUDIT → REPORT`로 확장했다. REPORT의 연결 LLM은 audit trace의 verified `stN` reference만 선택·정렬·그룹화하며 문장·claim·HTML을 새로 쓰지 않는다. strict draft가 provider/schema/ref/Audit 검증을 통과하지 못하면 같은 deterministic renderer가 `deterministic_fallback` artifact를 만든다. renderer는 canonical JSON/source hash, HTML/draft hash, escape·내부 URL allowlist, 1 MiB 출력 상한을 강제한다. REPORT 동안에도 lease heartbeat와 stage-boundary cancel을 유지하며, 최종 artifact는 03 §4.4의 단일 transaction에서 JSON report/job과 함께 확정한다.
...
> **구현 메모 (Phase 6 — tool JSON schema 확정, 2026-08-12):** 위 deferred("구체 JSON schema 본문 미확정")는 실제로 **4 tool 계열이 이미 추출·판정 골든으로 검증된 코드**에 존재함을 정직히 정리했다 (측정 없는 확정 금지 blueprint §20 — 골든 검증된 contract). `tool_schemas.py` — 명시적 JSON Schema + 검증기로 공개 계약 봉인. **Extractor claim**(`CLAIM_CANDIDATE_SCHEMA`, 05 §6·03 §4.2 — 필수 필드·confidence∈[0,1]·`provenance_ref` list[str], ADR-305 authoritative 진입 전 필수) · **canonical verdict**(`CANONICAL_VERDICT_SCHEMA`, 07 §4.2 — relation 7라벨 enum·confidence∈[0,1]) · **contradiction verdict**(`CONTRADICTION_VERDICT_SCHEMA`, 07 §5.2 — verdict/conflict_type enum·rationale 필수·confidence) · **synthesis report**(`SYNTHESIS_REPORT_SCHEMA`, 07 §3.8/§9 — subject_id/conclusion/statements/open_questions/audit). 각 `output_schema_version="1.0.0"` (07 §7 버전 관리 → [03] 저장 스키마 정합, contract test 대상 blueprint §15). 검증기는 read-only·결정적, 실패 시 `{valid:False, reason}` — 산출 실패는 quarantine 으로(07 §7 → 05/06). 스키마 실질 값 변경 없음(기존 validate_* 골든 그대로) → **Spec 1.0.0 유지**. TDD — `test_tool_schemas` 신규 19개(valid/invalid·enum·범위·provenance 타입·스키마 버전·read-only·결정성) — 스위트 948→**967개 통과**(회귀 0). **나머지 deferred:** API·저장 통합 contract 는 실제 API/저장 골든 확정 시. **Spec 그대로(1.0.0).**
